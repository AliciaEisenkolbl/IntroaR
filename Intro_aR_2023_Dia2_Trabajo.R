...
Produccion de Soja <- 5.5 #el esrito de variables no debe usar espacios entre palabras 
ProduccionDeSoja <- 5.5 #El Camello #Se debe usar letras en may{uscula}
Produccion_de_Soja <- 5.5 #La Serpiente #se puede usar guines bajo # no usar nada de acentos , Ñ u otros 
...
#nombre de columnas
colnames(DPS)
#colnames#primeros datos de la tabla
#datos unicos
head(DPS)
...
boxplot(NGranos_m2 ~ Variedad, data = DPS, 
        xlab = "Variedad", #nombre eje xx 
        ylab = "Numero de granos m2" , #nombre eje yy
        main = "Nuemero de granos de cultivo m2", 
        las = 2, #para rotar
        cex.names = 1)

# ~ es para separar las variables dependientes de las independientes 
...
ggplot(DPS, aes( x = NGranos_m2,  y = Rendimiento_Kgha, col =
                Variedad)) +
  geom_point() +
  ggtitle("Rendimiento Kg/ha vs. Numero de Granos por m2") +
  xlim(0,4000) 
