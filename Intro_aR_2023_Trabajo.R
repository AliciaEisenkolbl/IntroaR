...
getwd() #Donde estamos trabajando a
#Comentarios numeral es ALT 3
#### 4 numeral y espacio  hace un capitulo ####
print( "hola")
...
range(x) #contiene el mínimo y máximo  
?range
 #mediana
...
#si el vector tiene diferenres tipos, R los transforma a un solo tipo
...
X1 <- cbind(X1, C.4) #Agregar 4ta columna a X
...
names(data)[3] <- "tres"
data$tres <- NULL #eliminar columna3
#el simbolo de $ despues del nombre del data frame me da opcioness de las columnas 
data$tres = c(1,2,2)
data$cuatro = c(3,3,4)

# Reordenar marco de datos
data <- data.frame(id=1:3, peso=c(20, 27, 24), tamaño=c("pequeño", "grande", "mediano"))
data[c(1, 3, 2)] #corchete delante del marco de daatos y elecciona el numero se reorena 
...
as.Date() #para convertir a echa 
as.character()#para convertir a texto

#seleccionamenosla columna de D y convertimos en matriz 
...
Sepalos_largo > 6
#se nececita usar corchete para seleccionar datos
Sepalos_largo[Sepalos_largo > 6]
Petalos_largo[Petalos_largo > 5]
#para anidarr condiciones podemos usar operadores logicos and & y OR\ palito derecho
